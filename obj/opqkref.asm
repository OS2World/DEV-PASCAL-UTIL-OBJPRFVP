...
EscapeSequence  PROC FAR
        ...
        MOV     SI,offset Keys          ;SI points to Keys
        CMP     CL,165                  ;CL <= 165?
        JBE     EsSearch
        CMP     CL,0E9h                 ;CL >= $E9?
        JB      EsNull
        CMP     CL,0EFh                 ;CL <= $EF?
        JA      EsNull
        MOV     SI,offset Mouse         ;point to mouse table
        SUB     CL,0E9h                 ;make CL relative to $E9
        JMP     SHORT EsSearch          ;now do the search

EsNull:
        MOV     SI,offset Null          ;CS:SI points to Null
        JMP     SHORT EsDone

EsSearch:
        JCXZ    EsDone                  ;Character #0? We're done
        MOV     AX,CS
        MOV     DS,AX                   ;DS:SI points to Keys or Mouse
        CLD                             ;Forward direction
        SetZero AH                      ;Clear top half of length word
